...
// book-facility's own REMEMBER_KEY (backed by sports_complex/utils/
// guest_booking.py's issue_booking_remember_token/verify_booking_remember_
// token) - a separate localStorage key so a Book a Coach remember-token
// is never confused with the other guest booking flows.
...
createApp({
  ...
  methods: {
    ...
    loadSlots() {
      ...
      frappe.call(
        'sports_complex.sports_complex.doctype.training_session.training_session.get_coach_available_slots',
        { coach: this.selectedCoach.name, date: this.selectedDate }
      ).then((r) => {
